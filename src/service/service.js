...
const uploadFiles = async function(files, setProgress) {
    ...
    const onStoredChunk = (chunkSize) => {
        sendSize += chunkSize;
        let progress = sendSize/size*100 > 100 ? 100 : sendSize/size*100
    ...
    const getFileBlob = function(url) {
        return new Promise((resolve, reject) => {
            ...
            request.addEventListener("progress", (event) => {
                ...
                let progress = loaded*100/total > 100 ? 100 : loaded*100/total
                setProgress(progress)
            })
            request.onload = (res) => {
...
const getLinks = async function(cid) {
    ...
    if (!res.ok) {
        throw new Error(`failed to get links by ${cid} - [${res.status}] ${res.statusText}`)
    }
